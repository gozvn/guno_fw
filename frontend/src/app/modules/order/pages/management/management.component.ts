...
@Component({
    selector: 'app-order-page-management',
    templateUrl: './management.component.html',
    styleUrl: './management.component.scss'
})
export class OrderPageManagementComponent extends BaseComponent implements OnDestroy {
    selectedTabIndex = 0;
    ...
    displayedColumns: string[] = [
        'id', 'customerMobile', 'customerName', 'product', 'note', 'customerAddress', 'shipping', 'totalAmount', 'insertedAt', 'updatedAt', 'status'
    ];
    ...
    search(loadingState = true) {
        ...
        this.orderService.search(params).subscribe((result: any) => {
            this.orders = result?.orders ?? [];
            this.pageLength = result.count;

            const statsStatuses = result?.stats.statuses;

            this.orderStatuses.map((item: any) => {
                const statsStatus = statsStatuses.find((e: any) => e.status === item.id);
                item.count = statsStatus?.total_order ?? 0;
                return item;
            });

            this.totalOrders = statsStatuses.reduce((sum: number, item: any) => sum + (item.status === 7 ? 0 : item.total_order), 0);
        });
